...
window.layer = {
    ...
    load: function(parme, loadIcon){
        if(typeof parme === 'string'){
            return this.msg(parme, loadIcon, 16);
        } else {
            return $.layer({
                time: parme,
                loading: {type : loadIcon},
                bgcolor: loadIcon ? '#fff' : '',
                shade: loadIcon ? [0.1, '#000'] : [0],
                border: (loadIcon === 3 || !loadIcon) ? false : [6, 0.3, '#000'],
                type : 3,
                title : ['',false],
                closeBtn : [0 , false]
            });
...
Class.pt.callback = function(){
    ...
    layerE.find('.xubox_min').on('click', function(){
        ...
        config.min && config.min(that.index);
    });
